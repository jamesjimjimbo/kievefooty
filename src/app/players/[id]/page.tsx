...
type Pick={
  id:string;kind:"gotw"|"own"|"special";selected_outcome:"home"|"draw"|"away";stake:number|string;odds:number|string;is_correct:boolean|null;
...
export default async function PlayerPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{wall?:string}>}){
  ...
  const streakProgress=Math.min(currentFeaturedRun,3);const winsToNextBonus=Math.max(0,3-currentFeaturedRun);
  ...
  return <AppShell><main className="content player-page">
    ...
    <section className="player-picks-section">
      ...
      <div className="card player-score-explainer"><div><span>Match returns</span><b>{points(weeklyReturns)}</b></div><span className="score-operator">+</span><div className="score-adjustment-breakdown"><span>Bonuses &amp; adjustments</span><ul><li><small>Challenges</small><b>{points(challengePoints)}</b></li><li><small>Streak bonuses</small><b>{points(streakPoints)}</b></li>{otherAdjustments!==0&&<li><small>Competitions &amp; other</small><b>{points(otherAdjustments)}</b></li>}</ul></div><span className="score-operator">=</span><div className="score-total"><span>Full score</span><b>{points(fullScore)}</b></div><p>The third consecutive featured-game win earns +10. Every further win in the same streak earns another +10.</p></div>
      <section className="card player-streak-record"><header><div><p className="eyebrow">Featured-game streak</p><h3>{currentFeaturedRun} consecutive {currentFeaturedRun===1?"win":"wins"}</h3></div><div className="streak-earned"><Target size={17}/><span>{points(streakPoints)} bonus pts earned</span></div></header><div className="streak-progress"><div><span>{currentFeaturedRun>=3?"Bonus active":"Progress to +10"}</span><b>{streakProgress}/3</b></div><div className="streak-dots" aria-label={`${streakProgress} of 3 wins toward the streak bonus`}>{[0,1,2].map(index=><i className={index<streakProgress?"filled":""} key={index}/>)}</div><p>{currentFeaturedRun>=3?"Every additional consecutive featured-game win earns another +10.":`${winsToNextBonus} more consecutive ${winsToNextBonus===1?"win":"wins"} needed.`}</p></div>{streakEntries.length>0&&<div className="streak-awards">{streakEntries.map((entry,index)=><div key={`${entry.week?.number??"bonus"}-${index}`}><CheckCircle2 size={15}/><span>{entry.week?`Week ${entry.week.number} · ${entry.week.label}`:"Streak bonus"}</span><b>+{points(Number(entry.amount))}</b></div>)}</div>}</section>
      ...
      <div className="player-week-list">{weekly.length?weekly.map(submission=>{
        ...
        return <details className="card player-week-card" key={submission.id}><summary><div className="week-title"><span className="week-kicker">Week {submission.week?.number??"—"}</span><h3>{submission.week?.label??"Competition week"}</h3></div><div className="week-pick-preview">{(submission.picks??[]).map(pick=><span className={pick.is_correct===true?"won":pick.is_correct===false?"lost":"pending"} key={pick.id}>{pick.is_correct===true?<CheckCircle2 size={13}/>:pick.is_correct===false?<XCircle size={13}/>:<Clock3 size={13}/>}<b>{selectionLabel(pick)}</b></span>)}</div><div className={`week-return ${pending?"live":"final"}`}><small>{pending?"Return so far":"Final return"}</small><b>{points(weekReturn)} pts</b></div><ChevronDown className="week-expand" size={18}/></summary>
          <div className="player-pick-list">{(submission.picks??[]).map(pick=>{const returned=pickReturn(pick);const kindLabel=pick.kind==="gotw"?"Game of the Week":pick.kind==="special"?"Manager's Special":"Own choice";return <div className="player-pick-row" key={pick.id}><div className={`pick-result-icon ${pick.is_correct===true?"won":pick.is_correct===false?"lost":"pending"}`}>{pick.is_correct===true?<CheckCircle2 size={18}/>:pick.is_correct===false?<XCircle size={18}/>:<Clock3 size={18}/>}</div><div className="pick-summary"><span>{kindLabel}</span><b>{selectionLabel(pick)}</b><small>{pick.fixture?.home?.name??"Home"} vs {pick.fixture?.away?.name??"Away"}</small></div><div className="pick-math"><span>{points(Number(pick.stake))} pts × {Number(pick.odds).toFixed(2)}</span><b>{returned===null?"Pending":`${points(returned)} pts`}</b></div></div>})}</div>
