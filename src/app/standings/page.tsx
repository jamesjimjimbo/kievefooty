...
export default async function Page(){
  ...
  const [{data:first},{data:second},{data:overall},{data:projected},{data:accuracyData}]=await Promise.all([
    ...
    supabase.from("weekly_submissions").select("user_id,competition_weeks!inner(is_casino),picks(is_correct)").eq("competition_weeks.is_casino",false),
  ]);
  const map=new Map<string,StandingPlayer>();
  for(const row of overall??[])map.set(row.user_id,{id:row.user_id,name:row.display_name,initials:row.display_name.split(/\s+/).map((x:string)=>x[0]).join("").slice(0,2).toUpperCase(),first:0,second:0,overall:Number(row.score),projected:Number(row.score),seasonProjection:0,accuracyCorrect:0,accuracyTotal:0,me:row.user_id===user.id});
