...
export default async function Page(){
  ...
  const [{data:first},{data:second},{data:overall}]=await Promise.all([
    supabase.rpc("get_standings",{p_half:"first"}),supabase.rpc("get_standings",{p_half:"second"}),supabase.rpc("get_standings",{p_half:null}),
  ]);
  const map=new Map<string,StandingPlayer>();
  for(const row of overall??[])map.set(row.user_id,{id:row.user_id,name:row.display_name,initials:row.display_name.split(/\s+/).map((x:string)=>x[0]).join("").slice(0,2).toUpperCase(),first:0,second:0,overall:Number(row.score),me:row.user_id===user.id});
