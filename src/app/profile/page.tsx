import type { Metadata } from "next";import { redirect } from "next/navigation";import { AppShell } from "@/components/app-shell";import { createClient } from "@/lib/supabase/server";import { updatePreferences } from "./actions";
...
export default async function Page(){
 ...
 const name=profile?.display_name??user.email?.split("@")[0]??"Player";const initials=name.split(/\s+/).map((x:string)=>x[0]).join("").slice(0,2).toUpperCase();const balance=(ledger??[]).reduce((s,row)=>s+Number(row.amount),0);
 ...
 <section className="card"><div style={{display:"flex",alignItems:"center",gap:14}}><span className="avatar" style={{width:54,height:54,background:"var(--forest)",color:"white",fontSize:16}}>{initials}</span><div><h2 style={{margin:0}}>{name}</h2><p className="subtle" style={{margin:0}}>{profile?.email??user.email}{profile?.is_admin?" · Admin":""}</p></div></div></section>
