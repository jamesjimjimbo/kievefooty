...
export function StandingsBoard({players}:{players:StandingPlayer[]}){
 ...
 <div className="card standings-summary"><div><p className="eyebrow">Prize share</p><b>{tab==="overall"?"70%":"15%"} of the pot</b></div>{tab==="overall"?<label className="projection-toggle large"><input type="checkbox" checked={includeSeason} onChange={event=>setIncludeSeason(event.target.checked)}/><span/><div><b>Include season bets</b><small>Assume current results stand</small></div></label>:<span className="pill">Weekly bets only</span>}</div>
