...
export function StandingsBoard({players}:{players:StandingPlayer[]}){
 ...
 <section className="card table">{sorted.length?sorted.map((p,i)=><div className={`standing-row ${p.me?"me":""}`} key={p.id}><span className="rank">{i+1}</span><span className="player"><ClubCrest seed={p.id} label={p.name} size="sm"/><span>{p.name}{p.me&&<small className="you-label">You</small>}</span></span><span className="form">—</span><span className="points">{p[tab]>0?"+":""}{p[tab]}</span></div>):<div style={{padding:24,textAlign:"center"}}><b>No players yet</b><p className="subtle">The table fills as friends create accounts.</p></div>}</section>
