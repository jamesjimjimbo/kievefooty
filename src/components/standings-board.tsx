...
export type StandingHistoryRow={
  ...
  first_score:number;second_score:number;full_score:number;accuracy_correct:number;
};
...
function AccuracyCompetition({players}:{players:StandingPlayer[]}){
  ...
  return <article className="card standings-card"><CompetitionHeader title="Most Accurate Picker" prize="10%" description="Most correct picks, including FA Cup and both Casino periods."/><div className="table competition-table">{sorted.map((player,index)=><div className={`standing-row accuracy-row ${player.me?"me":""}`} key={player.id}><span className="rank">{index+1}</span><PlayerCell player={player}/><span className="accuracy-score"><b>{player.accuracyCorrect}</b><small>of {player.accuracyTotal}</small></span></div>)}</div></article>;
}
...
function StandingsHistory({rows}:{rows:StandingHistoryRow[]}){
  const [view,setView]=useState<"full_score"|"first_score"|"second_score"|"accuracy_correct">("full_score");
  const modes=[
    ...
    {key:"accuracy_correct" as const,label:"Accuracy",title:"Most Accurate Picker",unit:"correct picks"},
  ];
    ...
    <div className="history-chart-scroll">
      <svg className="history-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${mode.title} by Competition Week`}>
        ...
        {players.map((player,playerIndex)=>{
          ...
          return <g className="history-series" key={player.id}><path d={path} style={{stroke:color}}/>{points.map(point=><circle cx={x(point.index)} cy={y(point.score)} r="5" style={{fill:color}} tabIndex={0} key={point.week.number}><title>{player.name} - {point.week.label}: {view!=="accuracy_correct"&&point.score>0?"+":""}{point.score} {mode.unit}</title></circle>)}</g>;
        })}
      </svg>
    </div>
  </section>;
