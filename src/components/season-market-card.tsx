...
export function SeasonMarketCard({market}:{market:SeasonMarket}){
  ...
  return <article id={`market-${market.slug}`} className={`card market-card ${open?"":"market-closed"}`}>
    ...
    {showSearch&&<label className="market-search"><Search size={15}/><span className="sr-only">Search {market.title} options</span><input type="search" value={query} onChange={event=>setQuery(event.target.value)} placeholder={market.slug==="golden-boot"?"Search players":"Search clubs"}/></label>}
